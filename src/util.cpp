...
const char * const BITCOIN_CONF_FILENAME = "pushi.conf";
const char * const BITCOIN_PID_FILENAME = "pushid.pid";
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "pushi" is a composite category enabling all Pushi-related debug output
            if(ptrCategory->count(string("pushi"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\PushiCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\PushiCore
    // Mac: ~/Library/Application Support/PushiCore
    // Unix: ~/.pushicore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "PushiCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/PushiCore";
#else
    // Unix
    return pathRet / ".pushicore";
#endif
#endif
}
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty pushi.conf if it does not excist
    ...
    {
        // Don't overwrite existing settings so command line settings override pushi.conf
